...
def get(msg,seek=0):  # 从服务器端下载文件
    ...
    if len(msg) > 0:
        ...
        if feedback.startswith("FileTransfer|get|ready"):  # 代表服务器上文件存在，并且服务器已经准备好了发送此文件到客户端
            ...
            if os.path.isfile('../users/{}/{}'.format(user, os.path.basename(file_name))):
                rw = 'ab'
                with open('../users/{}/{}'.format(user, os.path.basename(file_name)), 'rb') as f:
                    seek = len(f.read())
                raw_input("请按回车，完成断点续传")
            else:
                rw = 'wb'
                seek = 0
            sk.send(str(seek))
...
def put(msg,seek=0) :
    ...
    if feedback.startswith("FileTransfer|put|ready"):  # 代表服务器上文件存在，并且服务器已经准备好了发送此文件到客户端
        ...
        if os.path.isfile('../users/{}/{}'.format(user , os.path.basename(file_name))):
            ...
            seek=0
        sk.send(str(seek))
        with open('../users/{}/{}'.format(user , os.path.basename(file_name)), rw) as f:  # 在ftp服务器建一个新文件来存这个要下载的文件内容
            print '--->', file_name
            i = 0
            while not file_size == recv_size:  # 只要文件总大小和已收到的大小不想等，就代表还没收完
                if file_size - recv_size > 1024:  # 文件总大下减已收到的大小等于还剩下没收到的大小，如果这个数大于1024，代表一次肯定收不完，那就还得多循环几次
