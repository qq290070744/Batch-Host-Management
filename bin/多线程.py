...
class ssh_host(object):
    ...
    def group(self):
        ...
        reCount = self.cur.execute("select * from host where host_grop='{}' ".format(group_name))
        nRet = self.cur.fetchall()
        #print (nRet)
        ...
        def exec(ip,user,password,prot,cmd):
            ...
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            # 连接服务器
            ssh.connect(hostname=ip, port=prot, username=user, password=password)
...
if __name__=="__main__":
    conn = pymysql.connect(host='127.0.0.1', user='root', passwd='root', db='ssh')
    cur = conn.cursor()
