#!/usr/bin/env python
# -*- coding:utf-8 -*-
import prettytable,MySQLdb,logging
...
while True:
    ...
    if enter=='1':
        ...
        group_name=raw_input("请输入想要新建的主机组名称:").strip()
        host_ip=raw_input("请输入你要添加到这个组的 host IP:").strip()
        user=raw_input("输入主机用户名:").strip()
        password=raw_input("输入主机password:").strip()
        port=raw_input("输入端口号:").strip()
        ...
        group_name=raw_input("请输入想要del的主机组名称:").strip()
        ...
        host_ip=raw_input("请输入要查看服务器的IP:").strip()
        ...
        group_name=raw_input("输入所属的组:").strip()
        user = raw_input("输入主机用户名:").strip()
        password = raw_input("输入主机password:").strip()
        port=raw_input("输入端口号:").strip()
        reCount = cur.execute('insert into host(host_ip,host_group,username,password,port) values(%s,%s,%s,%s,%s)', (host_ip, group_name,user,password,port))
        ...
        #host_ip=raw_input("输入IP:").strip()
        host_ip = raw_input("请输入想要del的主机ip:").strip()
        ...
        host_ip=raw_input("输入host IP:").strip()
        group_name=raw_input("输入修改后的组名:").strip()
        user=raw_input("输入修改后的username:").strip()
        password=raw_input("输入修改后的password:").strip()
        port=raw_input("输入修改后的 port:").strip()
